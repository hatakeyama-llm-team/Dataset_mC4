...
# # 文字の繰り返しをチェックする辞書
    ...
    for char in text:
        if char in char_count:
            ...
            char_count[char] = 1
    # 200回以上繰り返される文字があるかチェック
    repeated_chars = [char for char, count in char_count.items() if count >= 200]
...
def calc_char_dup_rate(items, text):
    ...
    dup_chars = sum(len(item) * (count - 1) for item, count in counter.items() if count > 1)
