...
def filter2(text, threshold=0.9, min_length=10,  ngram_threshold_2gram=0.20,ngram_threshold_3gram=0.20,ngram_threshold_4gram=0.20):
    ...
    pos_counter, words, word_counter = parts_count(text, return_word_count=True)
    meishi_and_symbol_counts = pos_counter['名詞'] + pos_counter['記号'] + pos_counter['補助記号']
    ...
    most_common_2gram_count = ngram_counts_2gram.most_common(1)[0][1] if ngram_counts_2gram else 0
    ...
    repeated_3grams = sum(count for count in ngram_counts_3gram.values() if count > 1)
    ...
    repeated_4grams = sum(count for count in ngram_counts_4gram.values() if count > 1)
